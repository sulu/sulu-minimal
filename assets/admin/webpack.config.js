...
module.exports = (env, argv) => { // eslint-disable-line no-undef
    ...
    const rootPath = env && env.root_path ? env.root_path : path.resolve(__dirname, '..', '..'); // eslint-disable-line no-undef

    const composerConfig = require(path.resolve(rootPath, 'composer.json')); // eslint-disable-line import/no-dynamic-require
    ...
    return {
        entry: entries,
        output: {
            path: path.resolve(rootPath, publicDir),
            filename: basePath + '/[name].[chunkhash].js',
        },
        devtool: argv.mode === 'development' ? 'eval-source-map' : 'source-map',
        plugins: [
            new CleanWebpackPlugin({
                cleanOnceBeforeBuildPatterns: [path.resolve(rootPath, publicDir, basePath)],
